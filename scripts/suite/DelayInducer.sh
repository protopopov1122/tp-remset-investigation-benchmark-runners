...
javac -d "$BINDIR" "$BENCHMARK_SUITE_BASE_DIR/DelayInducer.java"
...
run_bench () {
    ...
    if [[ -x "/usr/bin/time" ]]; then
        /usr/bin/time -v java DelayInducer 2>&1 | tee "$TMPFILE"
        ...
        bash -c 'time java DelayInducer' 2>&1 | tee "$TMPFILE"
