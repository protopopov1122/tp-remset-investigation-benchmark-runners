...
run_bench () {
    ...
    if [[ -x "/usr/bin/time" ]]; then
        /usr/bin/time -v "$JAVA_HOME/bin/java" $(java_gc_log_flags $GC_LOGS/$counter.log) DelayInducer 2>&1 | tee "$TMPFILE"
        ...
        bash -c "time $JAVA_HOME/bin/java $(java_gc_log_flags $GC_LOGS/$counter.log) DelayInducer" 2>&1 | tee "$TMPFILE"
