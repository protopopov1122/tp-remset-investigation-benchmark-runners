export BENCHMARK_SUITE_SCRIPT_DIR="$BENCHMARK_SUITE_DIR/scripts/suite"
declare -A BENCHMARK_SUITE=()
declare -A BENCHMARK_PARAMETERS=()

add_all_benchmarks_to_suite () {
    for benchmark_script in $BENCHMARK_SUITE_SCRIPT_DIR/*.sh; do
        ...
        if [[ "x${BENCHMARK_SUITE[$benchmark_name]}" == "x" ]]; then
            BENCHMARK_SUITE[$benchmark_name]="$script_path"
...
initialize_benchmark_suite () {
    while IFS=',' read -ra benchmark_specs; do
        for benchmark_spec in "${benchmark_specs[@]}"; do
            ...
            local script_path="$(realpath -m $BENCHMARK_SUITE_SCRIPT_DIR/$benchmark_name.sh)"
    ...
    info "$(printf 'Benchmarks matching spec \"%s\":' $BENCHMARK_SPEC)"
    for benchmark_name in "${!BENCHMARK_SUITE[@]}"; do
        local benchmark_params="${BENCHMARK_PARAMETERS[$benchmark_name]}"
        local benchmark_script="${BENCHMARK_SUITE[$benchmark_name]}"

        add_benchmark_to_env_info "$benchmark_name" "$benchmark_script" "$benchmark_params"
...
execute_benchmark_suite () {
    for benchmark_name in "${!BENCHMARK_SUITE[@]}"; do
        ...
        export BENCHMARK_RESULT_DIR="$BENCHMARK_SUITE_RESULTS/benchmarks/$benchmark_name"
        export BENCHMARK_TMPDIR="$BENCHMARK_SUITE_RESULTS/tmp"
